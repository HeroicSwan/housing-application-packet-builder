...
describe("data lifecycle acceptance", () => {
  it("creates and decrypts a complete export, then performs approved deletion with evidence", () => {
    ...
    try {
      const environment = { ...process.env, DATABASE_URL: `file:./${databaseName}`, DATA_MODE: "synthetic", LOCAL_STORAGE_ROOT: storageRoot, DATA_ENCRYPTION_KEY: Buffer.alloc(32, 7).toString("base64"), DATA_ENCRYPTION_KEY_ID: "lifecycle-test" };
