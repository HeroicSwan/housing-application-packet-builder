...
const runOllamaReplay = process.env.RUN_OLLAMA_120 === "true" && env.DOCUMENT_PROCESSOR === "ollama";

(runOllamaReplay ? it : it.skip)("processes 120 synthetic applicants through the local Ollama extraction API", async () => {
  const processor = getDocumentProcessor();
  const started = performance.now();
  const results: { index: number; fields: number; warnings: number; category: string }[] = [];
  for (let start = 1; start <= 120; start += 4) {
    const batch = await Promise.all(Array.from({ length: Math.min(4, 121 - start) }, async (_, offset) => {
      ...
      const result = await processor.processDocument({ filename: `synthetic-applicant-${index}.txt`, mimeType: "text/plain", bytes: new TextEncoder().encode(text), category: "SYNTHETIC_APPLICATION", dataClass: "SYNTHETIC" });
      return { index, fields: result.fields.length, warnings: result.warnings.length, category: result.category };
    }));
    results.push(...batch);
  }
