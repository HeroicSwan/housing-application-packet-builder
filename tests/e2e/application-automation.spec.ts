...
test("Jordan Rivera application is completed, generated, bundled, and approved", async ({ page }) => {
  ...
  const questions = page.getByRole("link", { name: "Complete remaining questions" });
  if (await questions.count()) {
    await questions.click();
    for (const [label, value] of [["Date of birth", "1990-05-08"], ["Current mailing address", "128 Harbor Avenue, Riverton, NY 10004"]] as const) { await expect(page.getByRole("heading", { name: label })).toBeVisible(); await page.getByLabel(label).fill(value); await page.getByRole("button", { name: "Save and continue" }).click(); }
    await expect(page.getByRole("heading", { name: "Preferred contact method" })).toBeVisible(); await page.getByLabel("Preferred contact method").selectOption("Phone"); await page.getByRole("button", { name: "Save and continue" }).click();
    for (const [label, value] of [["Emergency contact", "Elena Rivera, sister, (555) 014-2299"], ["Identification expiration date", "2028-04-30"]] as const) { await expect(page.getByRole("heading", { name: label })).toBeVisible(); await page.getByLabel(label).fill(value); await page.getByRole("button", { name: "Save and continue" }).click(); }
    await expect(page.getByRole("heading", { name: "Applicant consent confirmed" })).toBeVisible(); await page.getByRole("checkbox", { name: /Yes, confirmed/ }).check(); await page.getByRole("button", { name: "Save and continue" }).click();
    await page.getByRole("link", { name: "Review full application" }).click();
  } else {
    await page.getByRole("link", { name: "Review and generate" }).click();
  }

  await page.getByLabel("Applicant’s typed legal name").fill("Jordan Rivera");
  await page.getByLabel(/I intend this typed name/).check();
  await page.getByLabel(/I consent to the approved supporting documents/).check();
