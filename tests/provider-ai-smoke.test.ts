...
(enabled ? it : it.skip)("runs three synthetic extraction requests through the configured AI provider", async () => {
  ...
  for (let index = 1; index <= 3; index += 1) {
    ...
    const result = await processor.processDocument({ filename: `ai-smoke-${index}.txt`, mimeType: "text/plain", bytes: new TextEncoder().encode(text), category: "IDENTITY" });
    expect(result.fields.length).toBeGreaterThan(0);
    results.push({ fields: result.fields.length, warnings: result.warnings.length, category: result.category });
  }
  const model = env.DOCUMENT_PROCESSOR === "gemini" ? env.GEMINI_MODEL : env.DOCUMENT_PROCESSOR === "sambanova" ? env.SAMBANOVA_MODEL : env.DOCUMENT_PROCESSOR === "cerebras" ? env.CEREBRAS_MODEL : env.DOCUMENT_PROCESSOR === "mistral" ? env.MISTRAL_MODEL : env.DOCUMENT_PROCESSOR;
