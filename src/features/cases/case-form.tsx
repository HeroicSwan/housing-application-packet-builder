...
type Values = { legalName?: string | null; preferredName?: string | null; dateOfBirth?: string | null; preferredLanguage?: string | null; phone?: string | null; email?: string | null; currentLivingSituation?: string | null; accessibilityNeeds?: string | null; notes?: string | null; recordVersion?: string | number | null };
...
export function CaseForm({ action, initialValues = {}, submitLabel }: { action: Action; initialValues?: Values; submitLabel: string }) {
  const initialState: CaseFormState = { message: "", errors: {}, values: Object.fromEntries(Object.entries(initialValues).map(([key, value]) => [key, String(value ?? "")])) };
  const [state, formAction] = useActionState(action, initialState);
  const value = (name: keyof Values) => String(state.values[name] ?? initialValues[name] ?? "");
  return <form action={formAction} className="border bg-white p-6 sm:p-8">
    {initialValues.recordVersion !== undefined && <input type="hidden" name="recordVersion" value={value("recordVersion")} />}
    {state.duplicates?.length ? <div role="alert" className="mb-6 border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><p className="font-semibold">Possible duplicate cases</p><p className="mt-1">Open each match before creating another record. If this is a different person, submit again to confirm.</p><ul className="mt-3 space-y-1">{state.duplicates.map((item) => <li key={item.id}><Link className="font-medium underline" href={`/cases/${item.id}`}>{item.referenceNumber} — {item.legalName}</Link></li>)}</ul><input type="hidden" name="duplicateAcknowledged" value="true" /></div> : null}
    <div className="grid gap-6 sm:grid-cols-2"><Field label="Legal name" name="legalName" value={value("legalName")} error={state.errors.legalName?.[0]} required /><Field label="Preferred name" name="preferredName" value={value("preferredName")} error={state.errors.preferredName?.[0]} /><Field label="Date of birth" name="dateOfBirth" value={value("dateOfBirth")} error={state.errors.dateOfBirth?.[0]} type="date" /><Field label="Preferred language" name="preferredLanguage" value={value("preferredLanguage")} error={state.errors.preferredLanguage?.[0]} /><Field label="Phone" name="phone" value={value("phone")} error={state.errors.phone?.[0]} type="tel" /><Field label="Email" name="email" value={value("email")} error={state.errors.email?.[0]} type="email" /><Area label="Current living situation" name="currentLivingSituation" value={value("currentLivingSituation")} error={state.errors.currentLivingSituation?.[0]} placeholder="Use plain, factual language." /><Area label="Accessibility needs" name="accessibilityNeeds" value={value("accessibilityNeeds")} error={state.errors.accessibilityNeeds?.[0]} /><Area label="Case notes" name="notes" value={value("notes")} error={state.errors.notes?.[0]} /></div>
    <div className="mt-8 flex flex-wrap items-center justify-between gap-4"><p aria-live="polite" className={`text-sm ${Object.keys(state.errors).length || state.conflict || state.duplicates?.length ? "text-red-700" : "text-emerald-700"}`}>{state.message}</p><SubmitButton pendingLabel="Saving…">{state.duplicates?.length ? "Confirm and create case" : submitLabel}</SubmitButton></div>
  </form>;
}
