...
export default async function PacketVersionsPage({ params }: { params: Promise<{ id: string }> }) {
  activateOrganizationContext(await requireRole(["REVIEWER"])); const { id } = await params; const current = await db.applicationPacket.findUnique({ where: { id } }); if (!current) notFound(); const packets = await db.applicationPacket.findMany({ where: { clientCaseId: current.clientCaseId }, include: { clientCase: true }, orderBy: { version: "desc" } });
