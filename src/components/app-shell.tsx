...
function Navigation({ items, pathname, onNavigate }: { items: NavItem[]; pathname: string; onNavigate?: () => void }) {
  return <nav aria-label="Primary" className="space-y-1">{items.map(({ href, label, icon: Icon }) => {
    const active = pathname === href || (!["/dashboard", "/admin"].includes(href) && pathname.startsWith(`${href}/`));
...
export function AppShell({ user, dataMode, children }: { user: User; dataMode: "synthetic" | "production"; children: React.ReactNode }) {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);
  const links = ["REVIEWER", "SUPERVISOR"].includes(user.role) ? [{ href: "/dashboard", label: "Dashboard", icon: LayoutDashboard }, { href: "/review", label: "Review queue", icon: ClipboardList }] : user.role === "AUDITOR" ? [{ href: "/dashboard", label: "Dashboard", icon: LayoutDashboard }, { href: "/cases", label: "Read-only cases", icon: Files }] : user.role === "SUPPORT_READ_ONLY" ? [{ href: "/dashboard", label: "System status", icon: LayoutDashboard }] : user.role === "ADMIN" ? [{ href: "/dashboard", label: "Dashboard", icon: LayoutDashboard }, { href: "/admin", label: "Administration", icon: Settings }, { href: "/admin/setup", label: "Organization setup", icon: ShieldCheck }, { href: "/admin/programs", label: "Programs", icon: Files }, { href: "/admin/users", label: "Staff access", icon: Users }, { href: "/admin/data-governance", label: "Data governance", icon: ShieldCheck }] : [{ href: "/dashboard", label: "Dashboard", icon: LayoutDashboard }, { href: "/cases", label: "Cases", icon: Files }, { href: "/review", label: "Applications & review", icon: ClipboardList }];
  return <div className="portal-shell min-h-screen bg-[#f4f6f8]">
    ...
    <div className="mx-auto grid min-h-screen max-w-[1600px] md:grid-cols-[240px_minmax(0,1fr)]">
      <aside className="sticky top-0 hidden h-screen flex-col border-r bg-white px-4 py-6 md:flex"><Link href="/dashboard" className="flex items-center gap-3 px-3 text-sm font-semibold leading-5 tracking-[-0.01em]"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#102a43] text-[10px] font-bold text-white">HP</span><span>Housing Packet<br />Builder</span></Link><p className="mt-3 px-3 text-xs text-muted-foreground">Nonprofit case workspace</p><div className="mt-8"><Navigation items={links} pathname={pathname} /></div><div className="mt-auto"><UserBlock user={user} /></div></aside>
      <div className="min-w-0"><main className="mx-auto max-w-[1280px] px-4 py-7 sm:px-7 sm:py-9 lg:px-10 lg:py-11">{children}</main><footer className="mx-auto max-w-[1280px] border-t px-4 py-5 text-xs text-muted-foreground sm:px-7 lg:px-10"><div className="flex flex-wrap justify-between gap-3"><span>Housing Application Packet Builder</span><span><Settings className="mr-1 inline h-3.5 w-3.5" />{dataMode === "synthetic" ? "Synthetic demonstration" : "Managed production deployment"}</span></div></footer></div>
    </div>
  </div>;
}
