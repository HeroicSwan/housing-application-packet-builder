...
export async function generateApplicationOutput(draftId: string) {
  ...
  const values = result.draft.fields.filter((field) => field.templateField.pdfFieldName).map((field) => ({ pdfFieldName: field.templateField.pdfFieldName!, value: field.templateField.fieldType === "SIGNATURE" ? result.draft.signature?.signedName ?? "" : field.finalValue ?? "", fieldType: field.templateField.fieldType }));
